...
void AliAnalysisTaskUpcRhoPrime::UserCreateOutputObjects()
{
  ...
  // if (!man)
  //   ::Info("", "AliAnalysisManager is null");

  AliInputEventHandler *inputHandler =
      (AliInputEventHandler *)(man->GetInputEventHandler());
  // if (!inputHandler)
  //   ::Info("", "inputHandler is null");

  fPIDResponse = inputHandler->GetPIDResponse();
  // if (!fPIDResponse)
  // ::Info("", "fPIDResponse is null");
  ...
  fRhoTree->Branch("TrackHasPointOnITSLayer0", &TrackHasPointOnITSLayer0,
                   "TrackHasPointOnITSLayer0[4]/O");
  fRhoTree->Branch("TrackHasPointOnITSLayer1", &TrackHasPointOnITSLayer1,
                   "TrackHasPointOnITSLayer1[4]/O");

  fRhoTree->Branch("TrackITSModuleInner", &TrackITSModuleInner,
                   "TrackITSModuleInner[4]/I");
  fRhoTree->Branch("TrackITSModuleOuter", &TrackITSModuleOuter,
                   "TrackITSModuleOuter[4]/I");

  fListHist = new TList();
  fListHist->SetOwner();

  fHistTriggersPerRun = new TH1I("fHistTriggersPerRun", "fHistTriggersPerRun",
                                 50000, 240000.5, 290000.5);
...
void AliAnalysisTaskUpcRhoPrime::UserExec(Option_t *)
{
  ...
  for (Int_t itr = 0; itr < esd->GetNumberOfTracks(); itr++)
  {
    ...
    // TODO: add param for HasPointOnITSLayer
  ...
  // ::Info("", "%d nGoodTracks", nGoodTracks);

  if (nGoodTracks == 4)
  { // fill tree variables
    // cout<<"four good tracks"<<endl;
